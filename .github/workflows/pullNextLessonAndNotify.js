...
async function fetchLesson(kidocode, cube, token, index) {
...
async function pullNextLessonIntoStudentRepo(studentUsername, studentRepoName, lessonBranch, masterToken, repo, studentToken, _silent) {
    try {
        ...
        shell.exec(`git pull https://unclecode:${masterToken}@github.com/${repo}.git ${lessonBranch}`, { silent: _silent });
        // let cubeInfo = {};
...
async function pullNextLessonIntoChub(branch, lessonBranch, masterToken, qHub, qHubCube, cHub, repo, _silent) {
    try {
        shell.exec(`git checkout --orphan ${lessonBranch}`, { silent: _silent });
        shell.exec(`git rm -rf .`, { silent: _silent });
        shell.exec(`git pull https://unclecode:${masterToken}@github.com/${qHub}/${qHubCube}.git ${lessonBranch}`, { silent: _silent });
        shell.exec(`git add --all`, { silent: _silent });
        shell.exec(`git commit -m 'Add next lesson branch'`, { silent: _silent });
...
async function updateCube(cHub, qHub, repo, gitToken, branch) {
    ...
    try {
        ...
        if (!authRes.result) {
            ...
            await pullNextLessonIntoStudentRepo(studentUsername, studentRepoName, lessonBranch, masterToken, repo, studentToken, _silent);
